...
// Progress report cached for 1 hour (monthly data is slow-changing within the day)
...
export const getProgressReport = async (req: Request, res: Response) => {
  try {
    ...
    const cacheKey = userId.toString() + ":" + (monthParamRaw || "current");
    ...
    // Cache key includes userId + month so different months are cached independently
    ...
    const [moods, sessions, journals, insights] = await Promise.all([
      Mood.find({ userId, timestamp: { $gte: startDate, $lte: endDate } }).sort(
        { timestamp: 1 }
      ),
      ChatSession.find({
        userId,
    ...
    // Theme frequency from chat sessions
    ...
    Write in Indonesian (Bahasa Indonesia). Be warm and personal, not clinical.`;
      ...
      aiSummary =
        ...
        PROGRESS_CACHE.set(cacheKey, { data: report, cachedAt: Date.now() });
